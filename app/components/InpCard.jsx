...
const ValGroup = ( {val, change, className} ) => {
    ...
    return (
        <div className={`${className ? className : ''} flex w-4/6 rounded-full bg-white border-black items-center justify-center`}>
            <button onClick={ () => handleClick(-1)} className="w-1/4 py-1 text-gray-700 text-xl hover:bg-sky-500 hover:bg-opacity-30 hover:text-black transition-all duration-150 rounded-l-full">-</button>
            ...
            <button onClick={ () => handleClick(1)} className="w-1/4 py-1 text-gray-700 text-xl hover:bg-sky-500 hover:bg-opacity-30 hover:text-black transition-all duration-150 rounded-r-full">+</button>
        </div>
    )
}

const InpCard = ( {children, img, val, change, className, wid}) => {
    return ( 
        <div className={`${className ? className : ''} flex flex-col justify-center items-center`}>
            ...
            <ValGroup className="mt-4" val={val} change={change}></ValGroup>
        </div>
     );
}
